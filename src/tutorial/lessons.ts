...
function frontCoach(id: LessonId, ctx: Context, done: Record<string, boolean>): Coach {
  ...
  switch (id) {
    ...
    case "clear-ground": {
      ...
      // The Scrub plate once the installation is chosen, else the installation itself on the table. (The
      // last alternative is the retired ledger tag, kept only until src/core/tutorial.test.ts drops it.)
      const scrubAt = (key: string) => `#target-dock .scrub-button[data-scrub="${key}"] || #intent-layer [data-anchor-installation="${key}"] || .ledger-chip.is-installation[data-scrub="${key}"]`;
