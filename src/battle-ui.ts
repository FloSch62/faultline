...
function ledgerMarkup(r: RunState, p: CombatPreview): string {
  ...
  if (r.faultNode || r.faultLink)
    chips.push(`<span class="ledger-chip is-fault" data-tooltip="Faults last for this player turn. Hot Patch, Link Recovery or Fast Reroute clear them; a second channel keeps transmitting.">${icon("link", 13)} Fault · <b>${esc(pretty(r.faultNode || r.faultLink || ""))}</b></span>`);
  ...
  for (const m of r.malware) chips.push(`<button class="ledger-chip is-malware" data-scrub="${m.id}" data-tooltip="${esc(`Malware in ${zoneForNode(m).toUpperCase()}: −${RULES.malwarePenalty} transmission damage. Scrub it for ${RULES.scrubCost} energy (or click it on the table), or Purge Field its band.`)}" ${r.energy < RULES.scrubCost ? "disabled" : ""}>${icon("malware", 13)} Malware · ${title(zoneForNode(m))} <em>Scrub <b>${RULES.scrubCost}</b>${icon("bolt", 12)}</em></button>`);
