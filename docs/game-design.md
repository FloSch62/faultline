...
- **Playing a card** spends its energy and removes it from hand. Normal cards go to discard; Exhaust cards and tokens go to the exhausted pile for the rest of the encounter; armed protocols wait in the protocol slots; **daemons** become seals on the player plate and run until the encounter ends. Unplayed cards go to discard at end of turn, except Retain cards (kept) and Volatile ones (exhausted). While Kernel Panic is in hand you can play at most 3 cards that turn (console, scrub, repair and relocation are not card plays; deleting a Worm is).
...
- **ACL Gate**: Online, it blocks 2 of each breach and 1 of each strike like any firewall. It links to the nearest devices it is not already cabled to (distance ties: device ids).
...
- **Energy and the next turn (v5).** The energy orb reads `current / base`, the base being `turnEnergyBase`; energy above the base glows with a lit rise marker (`+N`). Its tooltip names the base, each energy relic (and the cap), PoE Injectors online and next turn's energy, cards and block with their sources (next-turn energy, Reserve Cell, injectors, next-turn draw, Cache Servers and Fanout, next-turn block, block carried, Grounded Core). The console command sits on the orb's sigil; the pile plates (draw, discard, exhaust, prepare) stand beside it.
...
- **The board.** The table is a Blender model (`blender/boards/`, `src/three/board.ts`): a board per stage (containers, violet lancet lenses or hazard hatches on the front fascia), a unique board per guardian, and for every other leader a crest set in its colour on its stage's board (the far rail's sigil, the near corners and a rail light chain). `boardFor` picks it once per encounter from the room, so a death or an arrival never swaps it; Field Training uses the stage's board. The containerlab medallion stands in the fascia's centre; NORTH, CENTER and SOUTH are stencilled into the deck and light with their band (aimed, holding a field, or under INCOMING, when the side-rail lamps blink). The camera frames the table low under the rail, lowers it in a tall window and raises it only as far as needed to keep the fascia clear of the hand.
