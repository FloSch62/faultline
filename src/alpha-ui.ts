...
import { combatPreview, FIELD_RULES, zoneForNode } from "./core/run.ts";
...
export function guideMarkup() {
  return `<span class="eyebrow">THE ARCHITECT'S FIELD GUIDE</span><h2>Your network is your weapon.</h2><p class="modal-intro">Three stages of seven sectors. Three guardians. One backbone to restore. A deck shaped by your decisions.</p><button class="gold-button guide-tutorial" data-action="tutorial">Play the optional tutorial ${icon("play",16)}</button><div class="guide-grid">
    ...
    <div><b>04</b><h3>Claim the ground</h3><p>Field cards strengthen a band for 3 turns. Hostile corrosion adds 2 incoming damage in an occupied band; suppression takes 3 damage from a route through it. Both last 2 turns. Cleanse with Purge Field or move your hardware. One allied and one hostile field fit in each band; recasting replaces your field. After transmission, the surviving enemy acts. Faults last one player turn. Temporary shield and burst expire; energy resets to <strong>5</strong> plus reserves, and you draw <strong>6</strong>. New hostile fields activate on the following turn, even when cast alongside a jam, sever, or breach. Stage II attacks gain +1 damage and Stage III attacks gain +2; enemy pressure grows every three turns, and the Core enrages at half integrity: +3 to strikes/breaches, and 2 damage alongside its faults.</p></div>
...
export function enemyDossierMarkup(run: RunState) {
  ...
  const forecast = combatPreview(run), intent = forecast.intent, pattern = ENEMIES[run.enemy!.id].pattern;
  return `<span class="eyebrow">THE FIELD JOURNAL · HOSTILE SIGNAL</span><h2>${story.name}</h2><p class="dossier-title">${story.title}</p><p>${story.motive}</p><div class="enemy-pattern">${pattern.map(({kind,field},i) => `<span class="${i === run.enemy!.turn % pattern.length ? "current" : ""}"><b>${i + 1}</b>${{strike: "Strike", breach: "Breach", jam: "Jam", sever: "Sever", corrupt: "Corrupt zone"}[kind]}${field ? `<small>+ ${FIELD_RULES[field].name}</small>` : ""}</span>`).join("")}</div><p class="enemy-telegraph">${intent ? story.telegraphs[intent.kind] ?? "" : ""}</p><h3>Build a countermeasure</h3><p>${story.counterplay}</p>${story.counterplay !== forecast.traitDescription ? `<p class="trait-explanation">${esc(forecast.traitDescription)}</p>` : ""}<p class="modal-intro">Each cycle repeats. Pressure adds 1 damage to strikes and breaches every three completed enemy actions. Stage II adds 1 attack damage; stage III adds 2. New hostile fields begin on the following player turn.</p><button class="text-button" data-action="help">Read the complete field guide →</button>`;
}
