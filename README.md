...
- **Keywords and Daemons.** Exhaust, Retain, Innate, Volatile and Armed explain themselves in a glossary tooltip on every card. A **Daemon** is paid once and runs for the rest of the encounter on the daemon strip above your protocols: Keepalive, Fabric Controller, Persistent State, Deep Queue, Botnet and fifteen more. Copies stack.
...
- Local autosave (version 5; earlier expeditions are not continued), a shared UTC daily seed, local run records, undo, card inspection, keyboard play and reduced motion.
...
Read the [complete implemented game design](docs/game-design.md) and the [story and world](docs/narrative.md). v5 is tuned against its own targets (32–42 % tactical wins per keeper at ascension 0); the [v4 balance probe](docs/balance-v4.json) and the [v3 baseline](docs/balance-v3.json) are kept as history. Simulated win rates are regression probes; human playtesting remains necessary to tune difficulty and enjoyment.

### Discover the signature cards

| Card | Rarity | Energy | Effect |
| --- | --- | --- | --- |
| Containerlab | Rare | 2 | Deploy an overclocked router cabled to ALPHA and OMEGA: a fresh 7-damage route. Exhaust. |
...
| Run a daemon | Play the daemon card; it joins the daemon strip above your protocols and runs for the encounter (right-click a plate to inspect it) |
...
- `src/ui.ts`, `src/card-marks.ts`, `src/battle-ui.ts`, `src/alpha-ui.ts`, `src/screens.ts` and their stylesheets — illustrated cards (owner frames, keyword markers, modified costs), the battle HUD (energy orb, daemon strip, port strip, transmission breakdown, escalation gauge, repair and scrub plates), inspection, journals (Details, Devices, the message dialog) and every expedition screen.
