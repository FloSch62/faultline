...
- **The far rail.** Up to three hostiles stand at the rail; the centre port is the v3 presentation, and escorts and adds stand to either side at a smaller scale. Under each sprite hangs a **rail plate** (name, three escalation pips for leaders, health bar with the forecast loss, the packet it will take this transmission; LETHAL in gold on a port forecast as defeated). Above each plate hangs the hostile's **intent badge** (`#intent-layer`, placed every frame from the 3D rail, one hostile or three): a big glyph, the number after every term (strike and breach) and a short verb with its target in the intent's colour (STRIKE 7, BREACH 5, JAM ROUTER1, CUT ROUTER1 ↔ SWITCH1, CORRODE NORTH, PLANT JAMMER · NORTH, OVERLOAD, CHARGE · ultimate next turn, the ultimate's own name), riders underneath (a field beside a fault, extra plantings, junk, healing), escalation diamonds when a level is reached or two actions away, RESTS · acts next phase for a dormant escort, FALLS THIS TURN (struck through in gold), FALLS · ACTS ANYWAY for a Spiteful hostile, BROKEN for an interrupted ultimate, and a dashed ARRIVES badge on a port an announced reinforcement will take. Badges hide while the enemy phase plays out. The **target** (the focus) wears a TARGET tag with the lit crest on its badge and brass reticle brackets around its rail plate that breathe (still under reduced motion); a click on a hostile's body, rail plate, badge or port row targets it. Hovering a hostile opens its card: the full forecast sentences, designations and rules, riders, shield terms, escalation, health now → after the transmission, and the deliveries and overflow that land on it. Hostiles still read as sprites with rim lighting, embers, anticipation and lunges; guardians are larger with a presence seal.
