...
export function enemyDossierMarkup(run: RunState) {
  ...
  return `<span class="eyebrow">THE FIELD JOURNAL · HOSTILE SIGNAL</span><h2>${story.name}</h2><p class="dossier-title">${story.title}</p><p>${story.motive}</p><div class="enemy-pattern">${story.pattern.map((kind,i) => `<span class="${i === run.enemy!.turn % story.pattern.length ? "current" : ""}"><b>${i + 1}</b>${{strike: "Strike", breach: "Breach", jam: "Jam", sever: "Sever"}[kind]}</span>`).join("")}</div><p class="enemy-telegraph">${intent ? story.telegraphs[intent.kind] ?? "" : ""}</p><h3>Build a countermeasure</h3><p>${story.counterplay}</p><p class="trait-explanation">${esc(combatPreview(run).traitDescription)}</p><p class="modal-intro">Each cycle repeats. Pressure adds 1 damage to strikes and breaches every three completed enemy actions.${run.enemy?.id === "core" ? " At or below half integrity: +3 to strikes/breaches and 2 damage alongside faults, starting with the next displayed intent." : ""}</p><button class="text-button" data-action="help">Read the complete field guide →</button>`;
}
